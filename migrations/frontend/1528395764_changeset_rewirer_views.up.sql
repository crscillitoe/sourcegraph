...
CREATE VIEW changeset_spec_tracked_mappings AS (
    SELECT changeset_specs.id AS changeset_spec_id, COALESCE(changesets.id, 0) AS changeset_id, changeset_specs.repo_id AS repo_id, changeset_specs.campaign_spec_id AS campaign_spec_id
...
CREATE VIEW changeset_spec_branch_mappings AS (
    SELECT changeset_specs.id AS changeset_spec_id, COALESCE(changesets.id, 0) AS changeset_id, changeset_specs.repo_id AS repo_id, changeset_specs.campaign_spec_id AS campaign_spec_id, changesets.owned_by_campaign_id as owner_campaign_id
    FROM changeset_specs
